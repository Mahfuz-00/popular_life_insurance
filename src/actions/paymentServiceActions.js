...
//live
const BKASH_GRANT_TOKEN_API = "https://tokenized.pay.bka.sh/v1.2.0-beta/tokenized/checkout/token/grant";
const BKASH_CREATE_PAYMENT_API = "https://tokenized.pay.bka.sh/v1.2.0-beta/tokenized/checkout/create";
const BKASH_EXECUTE_PAYMENT_API = "https://tokenized.pay.bka.sh/v1.2.0-beta/tokenized/checkout/execute";
const BKASH_QUERY_PAYMENT_API = "https://tokenized.pay.bka.sh/v1.2.0-beta/tokenized/checkout/payment/status";
...
export const BKASH_APP_SECRET_KEY = "uRtxZ97C0TKnHYPwjneinmkrrmAyGYFj0vLMC8g97coPR2YFrP4c";

export const bkashGetToken=async()=>{
...
export const bkashCreatePayment=async(token, amount, policyNo)=>{
...
export const bkashExecutePayment=async(token, paymentId)=>{
...
export const bkashPaymentStatus=async(token, paymentId)=>{
...
export const nagadPaymentUrl=async(postData)=>{
    try{
        const config = {
            headers: {
                'Content-Type': 'application/json',
        ...
        const { data } = await axios.post(`${API}/api/get-nagad-url`, postData, config);
        console.log("success: ", JSON.stringify(data.data.url));
