import { View, Text, ScrollView, ImageBackground, Image, Switch, TouchableOpacity, StyleSheet, ToastAndroid, Alert } from 'react-native';
...
import { bkashCreatePayment, bkashExecutePayment, bkashGetToken, nagadPaymentUrl } from '../../actions/paymentServiceActions';
...
var gatewayOptions = [  
  {label: <Image source={require('../../assets/nagad.png')} style={{width:80, height:35}}/>, value: 'nagad' },
  {label: <Image source={require('../../assets/bkash.png')} style={{width:80, height:35}}/>, value: 'bkash' },
  {label: <Image source={require('../../assets/otherCards.png')} style={{ height:35}}/>, value: 'ssl' }
];
...
const PhPayPremiumScreen = ({navigation, route}) => {
  ...
  const handleSubmit =async()=>{
    if(!isEnabled) return  ToastAndroid.show('Please agree terms & conditions', ToastAndroid.LONG);
    if( Number(amount) == 0 ) return  ToastAndroid.show('Amount can not be zero !', ToastAndroid.LONG);
    ...
    if(policyDetails.isLaps == true) return  ToastAndroid.show('Policy is lapsed !', ToastAndroid.LONG);
    if(policyDetails.isMaturity == true) return  ToastAndroid.show('Policy is matured !', ToastAndroid.LONG);
    ...
    if( Number(amount) % Number(policyDetails.totalpremium) != 0 ) return  ToastAndroid.show('Amount should be multiples of premium !', ToastAndroid.LONG);
        ...
        onNavigationStateChange={
            async data=>
            {
              if(JSON.stringify(data).includes('status=success')){
                ...
                const createExecuteResult = await bkashExecutePayment(bkashToken, bkashPaymentId);
                ...
                if(createExecuteResult.transactionStatus == "Completed"){     
                  ...
                  await AsyncStorage.setItem('syncPayments', JSON.stringify([...syncPayments, postData]));
                  const isSuccess = await userPayPremium(postData);
                  
                  if(isSuccess){                    
                    var syncPayments =  JSON.parse(await AsyncStorage.getItem('syncPayments')) ?? [];
                    updateSyncPayments = syncPayments.filter(item => item.transaction_no != postData.transaction_no);    
      ...
      <ImageBackground source={BackgroundImage} style={{flex:1}}>
        ...
        <ScrollView >
          <View>
            ...
            <Text style={[globalStyle.fontMedium, {color: '#FFF', marginTop: 15}]}>Choose Your Payment Method</Text>
            ...
            />
              ...
              keyboardType='numeric'
                ...
                labelStyle={[globalStyle.fontMedium, {color: '#FFF', marginTop: 5}]}
              />

              <View style={{flexDirection:'row', marginVertical:20, alignItems:'center', justifyContent:'center', width: '100%', flexWrap: 'wrap'}}>
                <Switch
                ...
                <Text style={[globalStyle.fontMedium, {fontSize:16}]}>I Agree to the </Text>
                <TouchableOpacity onPress={()=>{Linking.openURL("https://signup.sslcommerz.com/term-condition");}}>
                  <Text style={[globalStyle.fontMedium, {color: 'green', fontSize:16}]}>Terms & Conditions</Text>
                </TouchableOpacity>
              </View>
              ...
              <FilledButton
                  ...
                  style={{width: '40%', borderRadius: 50, alignSelf: 'center', marginVertical: 10}}
                  onPress={() => {
                      handleSubmit()
                  }}
              /> 
